...
bool Game::parseCoord(const string& token, int& r, int& c) {
    ...
    if (numPart.empty())
        cin >> numPart;   // 使用者打了空格，數字在下一個 token
    for (char ch : numPart)
...
void Game::placePlayerShips() {
    ...
    for (int i = 0; i < SHIP_COUNT; i++) {
        ...
        while (!placed) {
            ...
            cout << "Enter starting coordinate (e.g., A1 or A 1): ";
            string token;
            cin >> token;
...
bool Game::playerTurn() {
    ...
    while (keepAttacking) {
        cout << "\nEnemy Board:" << endl;
        enemyBoard.display(true);   // 隱藏敵方船位置
        cout << "Enter attack coordinate (e.g., A1 or A 1), or Q to quit: ";
        string token;
        cin >> token;
...
bool Game::aiTurn() {
    ...
    bool won = ai.takeTurn(playerBoard);
    cout << "\nYour Board:" << endl;
    playerBoard.display(false);
    return won;
}
